...
style_fig(fig1, height=350, title="Cantidad de experiencias publicadas por año", legend_title="Serie")
...
if len(exploded):
    ...
    style_fig(fig2, height=430, title=f"Experiencias por año, desglosadas por {dim_label.lower()}", legend_title=dim_label)
...
style_fig(fig3, height=320, title="Crecimiento acumulado del catálogo", legend_title="Serie")
...
with anim_tab2:
    ...
    mapa_fecha["categoria_macro_p"] = mapa_fecha["categoria_macro"].apply(_primary)
    ...
    fig_map = px.scatter_map(
        cum_map_df, lat="lat", lon="lon", color="categoria_macro_p",
        animation_frame="frame_anio", hover_name="titulo",
        hover_data={"lugar_texto": True, "lat": False, "lon": False},
        labels={"categoria_macro_p": "Categoría macro", "frame_anio": "Año"},
        zoom=1.6,
    )
    fig_map.update_layout(map_style="open-street-map", margin=dict(l=0, r=0, t=54, b=0))
    style_fig(
        fig_map, height=650,
        title="Expansión geográfica acumulada, año a año",
        legend_title="Categoría macro",
    )
    ...
    st.caption(
        f"Muestra todas las experiencias geolocalizadas con fecha ({mapa_fecha['item'].nunique()} de "
        f"{mapa_fecha['lugar_texto'].nunique()} lugares), acumuladas hasta el año seleccionado."
    )
